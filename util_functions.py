...
Inputs:
weights tensor
int value precision= [8,6,4,2] 
quant_method=['stochastic_rounding', 'normal_rounding', 'mid-rise']
bin_method=['uniform_range','histogram','prior_normal','None']

Output:
Quantized weights per layer denoted as 'weights_q'

Note:Bias has not been considered here
'''

def qat_quantizer(weights, precision, quant_method, bin_method='None'):

  #Stochastic Rounding
  if (quant_method == 'stochastic_rounding') and (bin_method !='None'):
    unique_values=qat_unique_value_generator(weights,precision,bin_method) 
    weights_q=stochastic_quant(weights, unique_values) 

  #Normal Rounding
  if (quant_method == 'normal_rounding') and (bin_method !='None'):
    unique_values=qat_unique_value_generator(weights,precision,bin_method) 
    weights_q=rounding_quant(weights, unique_values)

  # mid-rise quantization
  if (quant_method == 'mid-rise') and (bin_method =='None'):
    if len(weights) == 1:
        delta = weights / 2**precision
    else:
        delta = (torch.max(weights) - torch.min(weights)) / 2**precision
    weights_q = delta * (torch.floor(weights/delta) + 0.5)

  return weights_q


"""
Given weights per layer, the precision and approach, 
the function calculates a tensor of unique values per layer (weight+bias)
Returns unique values for that layer.
"""
...
def qat_unique_value_generator(weights, precision, bin_method):
    ...
    if bin_method =='uniform_range':
      ...
      unique_values=torch.linspace(start = min_val, end = max_val, steps = 2**precision) 

    #Histogram based bins. Midpoint of bin edges is used as unique values
    if bin_method == 'histogram':
      bin_edges=np.histogram_bin_edges(weights.detach().numpy(), bins=2**precision)
      bin_edges=list(map(lambda i, j : (i + j)/2, bin_edges[: -1], bin_edges[1: ])) 
      unique_values=torch.tensor(np.array(bin_edges))

    #Range based on quantiles of a normal distribution
    if bin_method == 'prior_normal':
      mean_val=torch.mean(weights).item()
      std_val=torch.std(weights).item()
      quantiles=np.linspace(0, 1, num=2**precision) #Quantile 
      quantiles=quantiles[:-1][1:] #Removing 1st and last element 
      unique_intm = list(map(lambda x : norm.ppf(x,loc=mean_val,scale=std_val), quantiles)) 
      unique_intm.append(torch.min(weights).item())
      unique_intm.append(torch.max(weights).item())
      unique_intm=torch.tensor(np.array(unique_intm))
      unique_values=unique_intm
